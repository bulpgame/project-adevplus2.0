...
const char * const BITCOIN_CONF_FILENAME = "adevplus20.conf";
const char * const BITCOIN_PID_FILENAME = "adevplus20d.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "adevplus20" is a composite category enabling all Adevplus20-related debug output
                if(ptrCategory->count(std::string("adevplus20"))) {
...
#else
    const char* pszModule = "adevplus20";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Adevplus20
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Adevplus20
    // Mac: ~/Library/Application Support/Adevplus20
    // Unix: ~/.adevplus20
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Adevplus20";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Adevplus20";
#else
    // Unix
    return pathRet / ".adevplus20";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty adevplus20.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override adevplus20.conf
