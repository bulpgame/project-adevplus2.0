...
*
*   Why is this needed?
*   - In Adevplus20 some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all Adevplus20 specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f ADV2\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
