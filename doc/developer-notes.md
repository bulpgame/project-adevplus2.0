...
Adevplus20 Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 5471 for connections and services them.
...
However, in open source software such as Adevplus20 Core, where everyone uses
...
pay attention to for reviewers of Adevplus20 Core code.

General Adevplus20 Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Adevplus20 Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Adevplus20 Core so that they can be integrated
