...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Adevplus20 database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Adevplus20 datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Adevplus20 datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Adevplus20 0.7 datadir."
    ;;
  3)
    echo "Detected Adevplus20 pre-0.8 datadir."
    ;;
  4)
    echo "Detected Adevplus20 0.8 datadir."
    ;;
esac
